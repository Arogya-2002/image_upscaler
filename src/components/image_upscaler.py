...
from src.entity.image_upscaler_config import ImageUpscalerConfig, ConfigEntity
from src.entity.image_upscaler_artifact import ImageUpscalerArtifact
        ...
        try:
            ...
            try:
                self.upscaler = pipeline(
                    "image-to-image",
                    model=self.image_upscaler_config.model_name,
                    device=0 if self.device == "cuda" else -1,
                    torch_dtype=torch.float16 if self.device == "cuda" else torch.float32
                )
                self.model_name = self.image_upscaler_config.model_name
                logging.info(f"Loaded model: {self.model_name}")
            except Exception as e:
                logging.warning(f"Failed to load model '{self.image_upscaler_config.model_name}', trying fallbacks. Error: {e}")
                fallback_models = [
